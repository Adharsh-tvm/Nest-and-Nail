  ...
  return (
    <div className="bg-white rounded-[2rem] shadow-sm border border-gray-100 flex flex-col h-full overflow-hidden relative">
      ...
      <div className="p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white z-10 sticky top-0">
        <div>
          {title && (
            <h3 className="font-black text-[#1B4332] text-2xl tracking-tight">{title}</h3>
          )}
          <p className="text-gray-400 text-xs font-medium mt-1">Manage your records effectively</p>
        </div>
            ...
            {isLoading ? (
                ...
                <tr key={i} className="animate-pulse">
                  {columns.map((_, j) => (
                    <td key={j} className="px-6 py-6">
                      ...
                      {j === 0 && <div className="h-2 bg-gray-50 rounded-full w-1/2"></div>}
                    </td>
                  ))}
                </tr>
              ))
              ...
              data.map((row: any, rowIndex: number) => (
                <tr
                  ...
                  className={`group transition-all duration-200 hover:bg-emerald-50/30 ${onRowClick ? "cursor-pointer hover:transform hover:scale-[1.01]" : ""
                    }`}
                >
              ...
              <tr>
                ...
                >
                  <div className="flex flex-col items-center justify-center">
                    ...
                    <h4 className="text-gray-900 font-bold mb-1">No records found</h4>
      ...
      <div className="p-6 md:p-8 flex items-center justify-between text-xs text-gray-500 bg-white border-t border-gray-50">
        <div className="flex items-center gap-2">
          <span className="font-medium bg-gray-100 px-2 py-1 rounded text-gray-600">{data ? data.length : 0}</span>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1 mx-2">
            <button className="w-8 h-8 flex items-center justify-center bg-[#1B4332] text-white rounded-lg shadow-sm font-bold">1</button>
            <button className="w-8 h-8 flex items-center justify-center hover:bg-gray-50 rounded-lg text-gray-600 font-medium transition-colors">2</button>
            <button className="w-8 h-8 flex items-center justify-center hover:bg-gray-50 rounded-lg text-gray-600 font-medium transition-colors">3</button>
